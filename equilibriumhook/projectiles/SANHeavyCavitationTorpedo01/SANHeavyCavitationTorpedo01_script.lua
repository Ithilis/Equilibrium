...
SANHeavyCavitationTorpedo01 = Class(SHeavyCavitationTorpedo) {
    ...
    OnEnterWater = function(self)
        ...
        --EQ: move the forkthread from here to OnCreate.
    end,
    ...
    OnCreate = function(self,inWater)
        ...
        --self:SetCollisionShape('Sphere', 0, 0, 0, 0.1) --why is this here? depth charges dont need a collision shape?
        self:ForkThread(self.ProjectileSplit) -- since they track the target, they rarely enter water vs surface ships so we just split regardless of in water or not.
    end,

    --EQ: increase the start delay to 2 seconds
